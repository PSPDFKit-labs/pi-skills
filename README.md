...
- `ssh` — delegate all tool operations (read, write, edit, bash, grep, find, ls) to a remote machine via SSH. Usage: `pi --ssh user@host` or `pi --ssh user@host:/remote/path`. Requires SSH key-based auth and `bash` on remote.
